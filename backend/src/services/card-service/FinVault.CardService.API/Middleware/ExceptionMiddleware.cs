...
// KeyNotFoundException        = 404 (card not found etc)
// InvalidOperationException   = 409 (card already removed etc)
// Everything else             = 500 (unexpected crash)

using System.Text.Json;

namespace FinVault.CardService.API.Middleware;
...
public class ExceptionMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            // Wrong password, invalid token, not your card etc
            // HTTP 401 = Unauthorized
            await WriteResponse(context, 401, ex.Message);
        }
        catch (KeyNotFoundException ex)
        {
            // Card not found, issuer not found etc
            // HTTP 404 = Not Found
            await WriteResponse(context, 404, ex.Message);
        }
        catch (InvalidOperationException ex)
        {
            // Card already removed, invalid status change etc
            // HTTP 409 = Conflict
